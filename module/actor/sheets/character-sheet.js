export class HxHCharacterSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    html.find("[data-action='roll-disc']").on("click", ev => {
      const el = ev.currentTarget; this.actor.rollDisciplina(el.dataset.key, el.dataset.canal);
    });
    html.find("[data-action='roll-ataque']").on("click", ev => {
      this.actor.rollAtaqueByIndex(Number(ev.currentTarget.dataset.index||0));
    });
    html.find("[data-action='add-arma']").on("click", async ev => {
      const armas = foundry.utils.duplicate(this.actor.system.armas || []);
