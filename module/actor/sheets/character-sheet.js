export class HxHCharacterSheet extends ActorSheet {
  static get defaultOptions(){
    return foundry.utils.mergeObject(super.defaultOptions, { classes:["hxh","sheet","actor"], width:1000, height:780, submitOnChange:true });
  }
  ...
  activateListeners(html){ super.activateListeners(html);
    ...
    html.find("[data-action='add-arma']").on("click", async ev => { const armas=foundry.utils.duplicate(this.actor.system.armas||[]); armas.push({nombre:"",dado:"1d6",stat:{key:"fuerza"},precision:0,efectos:""}); await this.actor.update({"system.armas":armas}); });
