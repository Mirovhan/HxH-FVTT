export class HxHActor extends Actor {
  ...
  rollStat(statKey){ const bono=Number(getProperty(this.system,`stats.${statKey}.bono`)||0);
    return (new Roll(`1d20 + ${bono}`)).roll({async:false}).toMessage({speaker:ChatMessage.getSpeaker({actor:this}),flavor:`Atributo: ${statKey}`});}
  rollHabilidad(i=0){ const h=(this.system?.habilidadesNaturales||[])[i]; if(!h)return;
    return (new Roll(`1d20 + ${Number(h.total||0)}`)).roll({async:false}).toMessage({speaker:ChatMessage.getSpeaker({actor:this}),flavor:`Habilidad: ${h.nombre}`});}
  rollSalvacion(k){ const tot=Number(getProperty(this.system,`salvaciones.${k}.total`)||0);
    return (new Roll(`1d20 + ${tot}`)).roll({async:false}).toMessage({speaker:ChatMessage.getSpeaker({actor:this}),flavor:`Salvación: ${k}`});}
  rollDisciplina(n, canal="D"){ const d=this.system?.disciplinas?.[n]; if(!d)return;
    const tot=canal==="D"?d.totalD:canal==="I"?d.totalI:d.totalC; const dice=d.dado||"1d4";
    return (new Roll(`${dice} + ${tot||0}`)).roll({async:false}).toMessage({speaker:ChatMessage.getSpeaker({actor:this}),flavor:`Disciplina: ${n} (${canal})`});}
  rollAtaqueByIndex(i=0){ const a=(this.system?.armas||[])[i]; if(!a)return;
    ...
    const r1=(new Roll(`1d20 + ${Number(this.system?.experticia||0)} + ${bono} + ${Number(a.precision||0)}`)).roll({async:false});
    const r2=(new Roll(`${a.dado||"1d6"} + ${val}`)).roll({async:false});
