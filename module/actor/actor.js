export class HxHActor extends Actor {
  prepareDerivedData() {
    ...
    data.evasiones = data.evasiones || {};
    data.evasiones.total = evBase + evRef + evArm;
  }

  /* ---- Tiradas (API v13) ---- */
  async rollStat(statKey){
    const bono = Number(getProperty(this.system, `stats.${statKey}.bono`) || 0);
  ...
  async rollHabilidad(index=0){
    const h = (this.system?.habilidadesNaturales||[])[index]; if (!h) return;
  ...
  async rollSalvacion(key){
    const tot = Number(getProperty(this.system, `salvaciones.${key}.total`) || 0);
    const roll = await (new Roll(`1d20 + ${tot}`)).evaluate({async:true});
    return roll.toMessage({ speaker: ChatMessage.getSpeaker({actor:this}), flavor: `Salvación: ${key}` });
  }

  async rollDisciplina(nombre, canal="D"){
    const d = this.system?.disciplinas?.[nombre]; if (!d) return;
    const tot  = canal==="D" ? d.totalD : canal==="I" ? d.totalI : d.totalC;
    const dice = d.dado || "1d4";
    const roll = await (new Roll(`${dice} + ${tot||0}`)).evaluate({async:true});
    return roll.toMessage({ speaker: ChatMessage.getSpeaker({actor:this}), flavor: `Disciplina: ${nombre} (${canal})` });
  }

  async rollAtaqueByIndex(index=0){
    const a = (this.system?.armas||[])[index]; if (!a) return;
    const k = a?.stat?.key || "fuerza";
    const val = Number(getProperty(this.system, `stats.${k}.valor`) || 0);
    ...
    return ChatMessage.create({
      ...
      content: `<b>${a.nombre||"Ataque"}</b><br>Precisión: ${r1.total}<br>Daño: ${r2.total}<br><small>${a.efectos||""}</small>`
    });
  }
}